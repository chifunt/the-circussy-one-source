...
{
    public sealed class GameHapticsPresenter : IStartable, IDisposable
    {
        ...
        {
            float multiplier = Mathf.Clamp(0.75f + healthPercentDamage * 3f, 0.75f, 1f);
            haptics.Play(GameHapticsCue.PlayerDamaged, multiplier);
        }

        private void OnPlayerHealed(int healedAmount, int maxHealth, float healthPercentHealed)
        {
            haptics.Play(GameHapticsCue.SnackEat);
        }
        ...
        private void OnLevelChanged(int level)
        {
            ...
            {
                haptics.Play(GameHapticsCue.LevelUp);
            }
        ...
        {
            haptics.Play(GameHapticsCue.GameOver);
        }
        ...
        private void OnPhaseChanged(RunPhase previous, RunPhase next)
        {
            ...
            {
                haptics.Play(GameHapticsCue.HeadlinerDefeated);
            }
        }
        ...
        {
            haptics.Play(GameHapticsCue.EncoreStarted);
        }
    }

    public sealed class GameUiHapticsPresenter : IStartable, ITickable, ITickableWhenPaused, IDisposable
    {
        ...
        public GameUiHapticsPresenter(IGameHaptics haptics)
        {
            this.haptics = haptics ?? NullGameHaptics.Instance;
        }
        ...
        private void BindInteractiveElement(VisualElement element)
        {
            ...
            EventCallback<ClickEvent> clicked = _ => haptics.Play(GameHapticsCue.UiClick);
            EventCallback<PointerEnterEvent> hovered = _ => haptics.Play(GameHapticsCue.UiHover);
